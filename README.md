...
| `diffusion_baselines.py` | **Propagation baselines.** Degree-only ranking, personalised PageRank seeded with MTI swept over the restart parameter, and a degree-preserving rewiring null under diffusion. Tests whether the GAT is reducible to a closed-form smoother. |
...
Every control applied to the model, in the order it was introduced. Each answers a
specific alternative explanation for the enrichment reported above.

| # | Control | What it rules out | Result |
|---|---|---|---|
| 1 | **Leakage guard** | the model reading its supervision target directly | MTI-derived columns excluded from the feature matrix and their absence asserted before training. Note the target remains a deterministic function of the retained features (see *Scope of the model*) |
| 2 | **Annotation removal** | recovering pathway enrichments from pre-supplied labels | AlphaFold, PANTHER, InterPro and UniProt features removed; the final model sees MR effect sizes and topology only |
| 3 | **GAT-vs-MLP ablation** (`graph_ablation.py`) | the ranking being reproducible without the graph | MLP significant on CausAge at K = 100 only; GAT holds at all three depths. MLP never significant on MoTrPAC |
| 4 | **Multi-seed stability** | a result resting on one initialisation | five seeds {0, 1, 7, 42, 123}; significance required in the majority. Reported figures are median-and-range |
| 5 | **Ranking-depth robustness** | a result resting on one choice of K | K = 100, 150, 200 reported throughout |
| 6 | **Universe correction** | a null computed over a pool the ranking cannot draw from | rank rows moved to the connected universe (N = 2,473, M = 30 / 831). CausAge unaffected; MoTrPAC weakens from p = 0.007 to 0.018 |
| 7 | **Degree-matched null** (`degree_matched_null.py`) | enrichment explained by node connectivity | passes. MoTrPAC strengthens to p = 0.012 at K = 100 (5/5 seeds); CausAge holds at all depths |
| 8 | **Model-free score composition** | the ranking being reproducible by arithmetic on the node table | `z(MTI) + z(1[n_layers >= 2])` recovers nothing: p = 0.119–0.442 (CausAge), 0.672–0.813 (MoTrPAC) |
| 9 | **Degree-only ranking** (`diffusion_baselines.py`) | connectivity alone reproducing the result | recovers **no** CausAge genes at any depth (x = 0). Does reach significance on MoTrPAC (p = 0.046 / 0.004 / 0.029) — see caveat below |
| 10 | **PPR diffusion baseline** (`diffusion_baselines.py`) | the GAT being an expensive closed-form smoother | plain propagation seeded with MTI recovers nothing at any restart parameter in the useful range (alpha = 0.15–0.85) |
| 11 | **Rewiring null under diffusion** (`diffusion_baselines.py`) | biological wiring carrying signal beyond the degree sequence, for a linear smoother | rewired graphs match or exceed the real graph for MoTrPAC under PPR, indicating the high-alpha MoTrPAC signal is degree rather than wiring |

```bash
python degree_matched_null.py --edges STRING_edges_REBUILD.tsv --ranks . \
    --refs ying_ageing_causal_genes.tsv motrpac_exercise_genes.tsv --nsim 10000

python diffusion_baselines.py --edges STRING_edges_REBUILD.tsv \
    --nodes MTI_REBUILD_5LAYER.tsv --ranks . \
    --refs ying_ageing_causal_genes.tsv motrpac_exercise_genes.tsv --nrewire 100
```

### Propagation baselines in full

Enrichment p-values, connected universe (N = 2,473), no learning in any row:

| Ranking | CausAge K=100/150/200 | MoTrPAC K=100/150/200 |
|---|---|---|
| MTI only | 0.344 / 0.552 / 0.712 | 0.812 / 0.892 / 0.813 |
| Degree only | 1.000 / 1.000 / 1.000 | **0.046** / **0.004** / **0.029** |
| PPR alpha = 0.15 | 0.344 / 0.552 / 0.442 | 0.747 / 0.892 / 0.914 |
| PPR alpha = 0.30 | 0.344 / 0.273 / 0.221 | 0.812 / 0.854 / 0.914 |
| PPR alpha = 0.50 | 0.344 / 0.273 / 0.221 | 0.866 / 0.854 / 0.914 |
| PPR alpha = 0.70 | 0.712 / 0.849 / 0.712 | 0.672 / 0.490 / 0.417 |
| PPR alpha = 0.85 | 1.000 / 1.000 / 0.712 | 0.145 / 0.139 / 0.163 |
| PPR alpha = 0.95 | 1.000 / 0.849 / 0.922 | 0.145 / **0.025** / **0.020** |

As alpha approaches 1 the PPR ranking converges toward degree, which is where the
MoTrPAC signal appears — consistent with the degree-only row rather than with
propagation of the MR signal.

### The two reference sets behave oppositely on degree
...
**CausAge is degree-poor** and degree ranking recovers none of it, so that arm
cannot be a connectivity artefact under any interpretation.

**MoTrPAC is degree-enriched** and degree ranking alone reaches significance on
it. The degree-matched null (control 7) shows the GAT ranking is not exploiting
this — its top 100 is marginally *less* connected than the universe — but the
exercise-responsive arm should be read with the confound stated. Whether the
degree enrichment reflects functional centrality or study bias cannot be settled
here: STRING's combined score aggregates curated-database and text-mining
channels, and the MoTrPAC reference is itself biased toward well-characterised,
highly expressed genes. Rebuilding the graph from experimental and co-expression
channels only would separate the two.

### Controls still outstanding

- **Degree-preserving rewiring of the GAT itself.** Control 11 rewires under
  diffusion, not under the trained model; retraining `multiseed.py` on rewired
  graphs is the direct test.
- **STRING restricted to experimental and co-expression channels**, to establish
  whether the degree structure is functional or bibliometric.
...
See **Controls** above for the full list of tests applied, and *Controls still
outstanding* for what remains.
