...
def find_mr_p_columns(df):
    """Detect MR p-value columns by name heuristics."""
...
def dedupe_keep_first(df, key="gene_symbol"):
    """Keep first occurrence of each gene_symbol (assumes df is already sorted by rank)."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Supervised MR-GAT-GNN trainer for exercise gene prioritisation."
    )
    parser.add_argument("--node_path", required=True,
                        help="Node TSV path (node table with MTI scores + precomputed features).")
    parser.add_argument("--edge_path", required=True,
                        help="Edge TSV path (kNN gene-gene edges).")
    parser.add_argument("--out_dir", default=None,
                        help="Directory to write outputs (default: same folder as node_path).")
    parser.add_argument("--epochs", type=int, default=300,
                        help="Number of training epochs (default: 300).")
    parser.add_argument("--lr", type=float, default=1e-3,
                        help="Adam learning rate (default: 1e-3).")
    parser.add_argument("--lambda_multi", type=float, default=0.3,
                        help="Weight for multi-layer BCE loss term (default: 0.3).")
    parser.add_argument("--alpha", type=float, default=1.0,
                        help="Weight for multi-omic support in hybrid score z-sum (default: 1.0).")
    parser.add_argument("--top_n", type=int, default=300,
                        help="Number of top-ranked genes to write to Top-N output files (default: 300).")
    ...
    # paths
    out_dir = args.out_dir if args.out_dir else os.path.dirname(args.node_path)
    ...
    # Feature columns: all numeric except MTI target fields
    ...
    # Drop legacy AlphaFold v2/v4 columns if present (prefer af_* from AFv6)
    ...
    print("\nFeature columns used (first 20):")
    print(" ", feature_cols[:20], "...")
    print(f"Total feature dims: {len(feature_cols)}")
    if old_af_cols:
        print(f"Dropped legacy AF columns: {old_af_cols}")
    if afv6_cols:
        print(f"AFv6 columns detected: {afv6_cols}")
    ...
    # Multi-layer classification label
    ...
    if has_multi:
        ...
        if n_pos > 0 and n_neg > 0:
            ...
            print(f"pos_weight for BCE: raw={raw_pw:.2f}, capped={capped_pw:.2f}")
        else:
    ...
    print(f"Edge src range [{edge_index[0].min().item()}..{edge_index[0].max().item()}]  "
          f"dst range [{edge_index[1].min().item()}..{edge_index[1].max().item()}]")

    data = Data(x=x, edge_index=edge_index).to(device)

    # Model + optimiser
    model = MTIMultiNet(
        in_dim=len(feature_cols), hidden_dim=64, out_dim=64, heads=4, dropout=0.2
    ).to(device)
    optim = torch.optim.Adam(model.parameters(), lr=args.lr)

    lambda_multi = args.lambda_multi if has_multi else 0.0
    print(f"\nStarting training — epochs={args.epochs}, lr={args.lr}, lambda_multi={lambda_multi}\n")
    ...
    # Deduplicate (sort by hybrid rank first, keep first occurrence of each gene)
    ...
    print(f"Detected MR p-value columns: {mr_p_cols}")
    nom_mask = any_mr_p_below(nodes, mr_p_cols, thresh=0.05)
    out_dedup["MR_nominal_any"] = nom_mask.values

    # Write full deduped ranking
    ...
    print(f"Saved deduped full ranking to:\n   {full_out_path}  ({len(out_dedup)} rows)")

    # Write MR-nominal subset
    ...
    print(f"Saved MR-nominal ranking to:\n   {mr_nom_path}  ({len(mr_nominal_df)} rows)")
    ...
    print(f"Saved Top-{topn} full:       {topn_full_path}  ({len(topn_full)} rows)")
    print(f"Saved Top-{topn} MR-nominal: {topn_nom_path}  ({len(topn_nom)} rows)")

    # Quick top-10 preview
    print("\nTop 10 genes (hybrid-ranked):")
    print(
        out_dedup[["gene_symbol", "GNN_EXERCISE_SUP_MTI_MULTI_rank", "GNN_EXERCISE_SUP_MTI_MULTI_rank_scaled"]]
